...
THEMES = {
    ...
    "dark": {
        ...
        # #565656 是量着调的：比卡片（#242424）清楚，又远暗于滑块（#E8E8E8）。
        "SWITCH_OFF":  "#565656",
        "SWITCH_KNOB": "#E8E8E8",
    },
}
...
def mix(c1, c2, t):
    ...
    **t 必须夹到 [0, 1]。** 弹簧动画会过冲（spatial 那一档能冲到 1.15），
    而颜色不能过冲：溢出通道格式化出来是三位十六进制 —— 深色下
    `mix("#A0A0A0", "#66CCFF", 1.05)` 得到 `'#63ce104'`，8 个字符的非法颜色。
    `create_oval` 于是抛 TclError，而异常发生在 Tk 回调里**会被吞掉**：画布
    刚被 `delete("all")` 清过，就停在空白状态 —— 表现是深色下"开"的开关
    整条消失（用户截图报过来的），浅色下因为目标色更暗、通道不溢出而看不出。
    浅色没事、深色出事，差别只在于"哪个通道本来就快到顶了"。

    颜色的这套语义跟官方那句一致：**表现力属于移动，不属于颜色。** 所以
    过冲只留给位置，颜色到端点就停。
    """
    if t <= 0.0:
        t = 0.0
    ...
    def _ch(i):
        v = int(round(a[i] + (b[i] - a[i]) * t))
        # 夹一次是保险：t 已经夹过，这里按理不会越界，但这个字符串是要交给
        # Tk 的，宁可多算一次也不能让它拿到非法的颜色。
        return 0 if v < 0 else (255 if v > 255 else v)

    return "#{:02x}{:02x}{:02x}".format(_ch(0), _ch(1), _ch(2))
