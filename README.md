...
```
OBS 开始推流 ─┐
              ├──→ 冷却闸门 ──→ OneBot HTTP ──→ NapCat ──→ QQ 群 @全体成员
按下快捷键  ─┘
```
...
| | |
|---|---|
| **精确触发** | 监听 OBS 的 `obs-websocket` 事件，只有真正开始推流才通知 |
| **通用兜底** | 全局快捷键（可自定义），覆盖 B站直播姬 / 抖音直播伴侣等无接口的软件 |
| **不占用你的 QQ** | 通过独立 QQ 副本实现双实例共存，通知器挂着时你照样能聊天 |
| **零依赖** | 只用 Python 标准库 + tkinter；连 obs-websocket 都是手写的 |
| **图形界面** | 一个大按钮搞定，不需要碰配置文件 |
| **可打包** | 附 PyInstaller 说明，可做成单文件 exe |
| **隐私友好** | 全部本地通信，WebUI 仅监听 127.0.0.1，聊天内容不落盘 |
...
三个触发源共用一个冷却闸门，所以同一次开播不会被发两遍：

1. **OBS 推流事件**（`triggers.ObsWatcher`）
   连上 OBS 内置的 `obs-websocket`，订阅 `StreamStateChanged`。
   端口和密码直接从 OBS 自己的配置文件读，**用户不需要手填**。

2. **全局快捷键**（`triggers.HotkeyListener`）
   Win32 `RegisterHotKey`。给直播姬、直播伴侣这类没开放接口的软件用。

3. **进程检测**（`live_notify.TriggerEngine`，默认关闭）
   检测到指定进程就触发。因为会误报，默认关掉，可在界面里打开。
