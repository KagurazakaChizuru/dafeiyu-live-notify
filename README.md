...
| 👀 **UP 主发新视频也通知** | 填个 UID 就行，不用登录 B站。**首次订阅不补发历史**，只播报订阅之后的新投稿 |
...
| 🔒 **隐私友好** | 除公开的开播状态 / 投稿列表外**全部本地通信**，不登录、不用 Cookie、聊天内容不落盘 |

> ⚠️ 程序只有两处外部网络请求，都在 B站，都是**公开接口**：
> 开启「直播间开播时通知」后定期读你自己直播间的开播状态；开启
> 「UP 主发新视频时通知」后定期读你订阅的 UP 主的投稿列表。
> 两者都不登录、不带 Cookie、不携带任何身份信息，默认直连不走代理
> （投稿接口需要匿名签名，见 `bili.py` 的说明）。介意的话关掉即可，
> 其余触发源完全在本地工作。
