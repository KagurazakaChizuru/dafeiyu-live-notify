...
| **[技术文档](docs/技术文档.md)** | 想读懂设计的人 | 架构分层、触发语义矩阵、OneBot / obs-websocket 接口规格、配置 Schema，以及实现中实际踩到的陷阱与对策 |
| **[开发文档](docs/开发文档.md)** | 要改代码的人 | 代码地图、必须遵守的工程约定、调试手段、扩展指南、打包发版 |
| **[更新记录](CHANGELOG.md)** | 所有人 | 每个版本改了什么 |

第一次用，直接看 [使用文档](docs/使用文档.md) 就够了。当前版本 **v1.1.0**。
...
| **说清楚在玩什么** | 自动识别当前游戏写进通知（「正在玩《三角洲》」）。看窗口和直播姬场景配置，**不截图、不上传画面** |
| **带封面** | 附一张直播间封面小图，只有几 KB（用 B站图床的缩放参数，不下载不处理） |
| **会再喊一次** | 开播 30 分 / 1 小时各补一条，给没看到的人第二次机会；有硬性条数上限 |
| **下播提示** | 自动检测下播，附本次直播时长和人气峰值；带离线宽限期，断流重连不会误报 |
